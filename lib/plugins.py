...
def init_plugins(config, local):
    ...
    if local:
        ...
        imp.load_module('electrum-drk_plugins', fp, pathname, description)
        plugin_modules = map(lambda name: imp.load_source('electrum-drk_plugins.'+name, os.path.join(pathname,name+'.py')), plugin_names)
    else:
        import electrum-drk_plugins
        plugin_names = [name for a, name, b in pkgutil.iter_modules(electrum-drk_plugins.__path__)]
        plugin_modules = [ __import__('electrum-drk_plugins.'+name, fromlist=['electrum-drk_plugins']) for name in plugin_names]
